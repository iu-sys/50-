...
const KanaQuiz = () => {
  ...
  const [currentKana, setCurrentKana] = useState({
    kana: '',       // 題目假名 (平假或片假)，第一、二、三、四關共用
    romaji: ''      // 對應的羅馬拼音
  });
  const [lastRomaji, setLastRomaji] = useState('');      // 第一關避免重複
  const [lastKanas, setLastKanas] = useState([]);        // 第二/三關避免同一題
  const [options, setOptions] = useState([]);            // 按鈕選項 (string[] 或 {kana,romaji}[])
  const [level, setLevel] = useState(0);                 // 當前關卡 0~3
  const [selectedGroups, setSelectedGroups] = useState([]); // 使用者選的假名組
  const [score, setScore] = useState(0);                 // 當前關卡分數
  const [wrongCount, setWrongCount] = useState(0);       // 錯誤累計
  const [userAnswer, setUserAnswer] = useState('');      // 第3/4關輸入
  const [showAnswer, setShowAnswer] = useState(false);   // 顯示正確答案
  const [lastWrong, setLastWrong] = useState(false);     // 錯誤後鎖定
  ...
  const renderGroupSelector = () => {
    ...
    const labels = { hiragana: '平假名 (ひらがな)', katakana: '片假名 (カタカナ)' };
    const rows = {
    ...
    const renderSection = (type, list, title) => (
      <Box sx={{ mb:4 }}>
      ...
      <Box sx={{ mt:2 }}>
        {types.map(type=>(
          <Box key={type} sx={{ mb:6 }}>
            ...
            {renderSection(type, rows.daku, '濁音')}
            {renderSection(type, rows.handaku, '半濁音')}
            {renderSection(type, kRows.basic, '基本音')}
            {renderSection(type, kRows.daku, '濁音')}
            {renderSection(type, kRows.handaku, '半濁音')}
          </Box>
        ))}
      </Box>
    );
                    ...
                    <Grid item xs={6} key={i}>
                      <Button
                        ...
                        sx={{
                          fontSize:'2rem',
                          height:60,
                          bgcolor: showAnswer && opt.kana===currentKana.kana ? 'success.light':'inherit'
                        }}
                      >
                        {opt.kana}
                      </Button>
